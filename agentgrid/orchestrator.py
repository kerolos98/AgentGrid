...
from .utils.utils import parse_doc, map_mcp_to_litellm
...
class Orchestrator:
    ...
    def pull_from_server(self, server):
        """
        Pulls tools/resources/prompts from an existing MCP server.
        Converts native MCP SDK types to agentgrid framework objects safely.
        """
        for comp in server.components:
            # ==========================================================
            # 🛠️ 1. Handle Tools
            # ==========================================================
            if isinstance(comp, MCPTool):
                self.tools_list.append(comp)
            elif hasattr(comp, 'name') and (isinstance(comp, mcp_types.Tool) or type(comp).__name__ == "Tool"):
                try:
                    # Fallback Step A: Attempt standard positional instantiation if keywords are rejected
                    mcp_tool = MCPTool(
                        comp.name, 
                        getattr(comp, 'description', ''), 
                        getattr(comp, 'inputSchema', {'type': 'object', 'properties': {}})
                    )
                except TypeError:
                    # Fallback Step B: Create an empty instance and bind properties dynamically
                    mcp_tool = MCPTool.__new__(MCPTool)
                    mcp_tool.name = comp.name
                    mcp_tool.description = getattr(comp, 'description', '')
                    mcp_tool.inputSchema = getattr(comp, 'inputSchema', {'type': 'object', 'properties': {}})
            ...
            # 📦 2. Handle Resources
            # ==========================================================
            elif isinstance(comp, MCPResource):
                self.resources_list.append(comp)
            elif hasattr(comp, 'uri') and (isinstance(comp, mcp_types.Resource) or type(comp).__name__ == "Resource"):
                try:
                    mcp_res = MCPResource(
                        comp.name, 
                        str(comp.uri), 
                    ...
                    mcp_res = MCPResource.__new__(MCPResource)
                    mcp_res.name = comp.name
                    mcp_res.uri = str(comp.uri)
                    mcp_res.description = getattr(comp, 'description', '')
                    
                self.resources_list.append(mcp_res)

            # ==========================================================
            # 📝 3. Handle Prompts
            # ==========================================================
            elif isinstance(comp, MCPPrompt):
                self.prompts_list.append(comp)
            elif hasattr(comp, 'name') and (isinstance(comp, mcp_types.Prompt) or type(comp).__name__ == "Prompt"):
                try:
                    mcp_prompt = MCPPrompt(
                        comp.name, 
                        getattr(comp, 'description', ''), 
                        getattr(comp, 'arguments', [])
                    )
                except TypeError:
                    mcp_prompt = MCPPrompt.__new__(MCPPrompt)
                    mcp_prompt.name = comp.name
                    mcp_prompt.description = getattr(comp, 'description', '')
                    mcp_prompt.arguments = getattr(comp, 'arguments', [])
                    
                self.prompts_list.append(mcp_prompt)
    ...
    def add_agent(self, name: str, model: str):
        server, mode = self.register_to_mcp()
    ...
    def _tool_schema(self, tool):
        if self.agents[next(iter(self.agents))].server_meta.mode == "stdio":
            return map_mcp_to_litellm(tool)
    ...
    def run_agent(
        ...
        while step < max_steps:
            ...
            for call in tool_calls:
                ...
                if self.agents[agent_name].server_meta.mode == "stdio":
                    state.log("tool_call_error", "Tool calls not supported in stdio mode")
                    result = self.server.call_tool(tool_name, args)
                elif self.agents[agent_name].server_meta.mode == "local":
